"""
RayIoU / RayPQ metrics.
Computation is kept aligned with SparseOcc/loaders/ray_metrics.py.
"""

import copy
import math
import logging
import os
import tempfile
from pathlib import Path
from typing import Any, Optional

import numpy as np
import torch
from torch.utils.cpp_extension import load
from tqdm import tqdm

from FreeOcc.eval.ray_pq import Metric_RayPQ

try:
    from prettytable import PrettyTable
except Exception:  # pragma: no cover - optional dependency fallback
    class PrettyTable:  # type: ignore[override]
        def __init__(self, field_names):
            self.field_names = field_names
            self.rows = []
            self.float_format = ".3"

        def add_row(self, row, divider=False):
            self.rows.append(row)

        def __str__(self):
            lines = [" | ".join([str(x) for x in self.field_names])]
            for row in self.rows:
                lines.append(" | ".join([str(x) for x in row]))
            return "\n".join(lines)


_pc_range = [-40, -40, -1.0, 40, 40, 5.4]
_voxel_size = 0.4
_dvr_module: Optional[Any] = None
_lidar_rays_tensor: Optional[torch.Tensor] = None


def make_torch_extensions_writable():
    cache_root = Path.home() / ".cache" / "torch_extensions"
    try:
        cache_root.mkdir(parents=True, exist_ok=True)
        probe_path = cache_root / ".roadocc_write_probe"
        with open(probe_path, "w", encoding="utf-8") as file_handle:
            file_handle.write("ok")
        os.remove(probe_path)
        return
    except Exception:
        fallback_root = Path(tempfile.gettempdir()) / "roadocc_torch_extensions"
        fallback_root.mkdir(parents=True, exist_ok=True)
        os.environ["TORCH_EXTENSIONS_DIR"] = str(fallback_root)
        logging.warning(
            "Default torch extension cache is not writable. Using TORCH_EXTENSIONS_DIR=%s",
            fallback_root,
        )


def resolve_dvr_source_paths():
    repo_root = Path(__file__).resolve().parents[2]
    candidate_dirs = [
        repo_root / "lib" / "dvr",
        repo_root / "FreeOcc" / "lib" / "dvr",
        repo_root / "GaussianFlowOcc" / "lib" / "dvr",
        repo_root / "SparseOcc" / "lib" / "dvr",
    ]
    for candidate_dir in candidate_dirs:
        cpp_path = candidate_dir / "dvr.cpp"
        cu_path = candidate_dir / "dvr.cu"
        if cpp_path.exists() and cu_path.exists():
            return str(cpp_path), str(cu_path)
    raise FileNotFoundError(
        "Could not find dvr.cpp/dvr.cu in lib/dvr, FreeOcc/lib/dvr, GaussianFlowOcc/lib/dvr, or SparseOcc/lib/dvr."
    )


def get_dvr_module():
    global _dvr_module
    if _dvr_module is None:
        make_torch_extensions_writable()
        cpp_path, cu_path = resolve_dvr_source_paths()
        logging.info("Loading ray DVR extension from cpp=%s cu=%s", cpp_path, cu_path)
        _dvr_module = load(
            "dvr_roadocc_v2",
            sources=[cpp_path, cu_path],
            verbose=True,
            extra_cuda_cflags=["-allow-unsupported-compiler"],
        )
    return _dvr_module


def get_lidar_rays_tensor() -> torch.Tensor:
    global _lidar_rays_tensor
    if _lidar_rays_tensor is None:
        _lidar_rays_tensor = torch.from_numpy(generate_lidar_rays())
    return _lidar_rays_tensor


def get_rendered_pcds(origin, points, tindex, pred_dist):
    pcds = []

    for t in range(len(origin)):
        mask = tindex == t
        if not mask.any():
            continue
        _pts = points[mask, :3]
        v = _pts - origin[t][None, :]
        d = v / np.sqrt((v ** 2).sum(axis=1, keepdims=True))
        pred_pts = origin[t][None, :] + d * pred_dist[mask][:, None]
        pcds.append(torch.from_numpy(pred_pts))

    return pcds


def meshgrid3d(occ_size, pc_range):
    W, H, D = occ_size

    xs = torch.linspace(0.5, W - 0.5, W).view(W, 1, 1).expand(W, H, D) / W
    ys = torch.linspace(0.5, H - 0.5, H).view(1, H, 1).expand(W, H, D) / H
    zs = torch.linspace(0.5, D - 0.5, D).view(1, 1, D).expand(W, H, D) / D
    xs = xs * (pc_range[3] - pc_range[0]) + pc_range[0]
    ys = ys * (pc_range[4] - pc_range[1]) + pc_range[1]
    zs = zs * (pc_range[5] - pc_range[2]) + pc_range[2]
    xyz = torch.stack((xs, ys, zs), -1)

    return xyz


def generate_lidar_rays():
    pitch_angles = []
    for k in range(10):
        angle = math.pi / 2 - math.atan(k + 1)
        pitch_angles.append(-angle)

    while pitch_angles[-1] < 0.21:
        delta = pitch_angles[-1] - pitch_angles[-2]
        pitch_angles.append(pitch_angles[-1] + delta)

    lidar_rays = []
    for pitch_angle in pitch_angles:
        for azimuth_angle in np.arange(0, 360, 1):
            azimuth_angle = np.deg2rad(azimuth_angle)

            x = np.cos(pitch_angle) * np.cos(azimuth_angle)
            y = np.cos(pitch_angle) * np.sin(azimuth_angle)
            z = np.sin(pitch_angle)

            lidar_rays.append((x, y, z))

    return np.array(lidar_rays, dtype=np.float32)


def process_one_sample(
    sem_pred,
    lidar_rays,
    output_origin,
    instance_pred=None,
    occ_class_names=None,
):
    dvr = get_dvr_module()

    T = output_origin.shape[1]
    pred_pcds_t = []

    free_id = len(occ_class_names) - 1
    occ_pred = copy.deepcopy(sem_pred)
    occ_pred[sem_pred < free_id] = 1
    occ_pred[sem_pred == free_id] = 0
    occ_pred = occ_pred.permute(2, 1, 0)
    occ_pred = occ_pred[None, None, :].contiguous().float()

    offset = torch.Tensor(_pc_range[:3])[None, None, :]
    scaler = torch.Tensor([_voxel_size] * 3)[None, None, :]

    lidar_tindex = torch.zeros([1, lidar_rays.shape[0]])

    for t in range(T):
        lidar_origin = output_origin[:, t : t + 1, :]
        lidar_endpts = lidar_rays[None] + lidar_origin

        output_origin_render = ((lidar_origin - offset) / scaler).float()
        output_points_render = ((lidar_endpts - offset) / scaler).float()
        output_tindex_render = lidar_tindex

        with torch.no_grad():
            pred_dist, _, coord_index = dvr.render_forward(
                occ_pred.cuda(),
                output_origin_render.cuda(),
                output_points_render.cuda(),
                output_tindex_render.cuda(),
                [1, 16, 200, 200],
                "test",
            )
            pred_dist *= _voxel_size

        pred_pcds = get_rendered_pcds(
            lidar_origin[0].cpu().numpy(),
            lidar_endpts[0].cpu().numpy(),
            lidar_tindex[0].cpu().numpy(),
            pred_dist[0].cpu().numpy(),
        )
        coord_index = coord_index[0, :, :].long().cpu()

        pred_label = sem_pred[coord_index[:, 0], coord_index[:, 1], coord_index[:, 2]][:, None]
        pred_dist = pred_dist[0, :, None].cpu()

        if instance_pred is not None:
            pred_instance = instance_pred[coord_index[:, 0], coord_index[:, 1], coord_index[:, 2]][:, None]
            pred_pcds = torch.cat([pred_label.float(), pred_instance.float(), pred_dist], dim=-1)
        else:
            pred_pcds = torch.cat([pred_label.float(), pred_dist], dim=-1)

        pred_pcds_t.append(pred_pcds)

    pred_pcds_t = torch.cat(pred_pcds_t, dim=0)
    return pred_pcds_t.numpy()


def compute_visible_voxel_mask_from_lidar_rays(
    sem_grid,
    output_origin,
    free_class_id=17,
    include_free=False,
):
    """Compute a visibility mask using the exact DVR ray-tracing path used by ray metrics.

    Args:
        sem_grid: Occupancy semantic voxel grid with shape (200, 200, 16).
        output_origin: Tensor/array shaped (1, T, 3) or (T, 3), same as ray metrics origins.
        free_class_id: Semantic ID used as free space.
        include_free: If True, also mark rays whose first hit is free class.
    """
    dvr = get_dvr_module()
    lidar_rays = get_lidar_rays_tensor()
    if not torch.cuda.is_available():
        raise RuntimeError(
            "compute_visible_voxel_mask_from_lidar_rays requires CUDA because DVR rendering runs on GPU."
        )

    sem_grid = torch.as_tensor(sem_grid, dtype=torch.int64)
    if sem_grid.ndim != 3:
        raise ValueError(f"sem_grid must be 3D (W,H,Z), got shape={tuple(sem_grid.shape)}")

    output_origin = torch.as_tensor(output_origin, dtype=torch.float32)
    if output_origin.ndim == 2:
        output_origin = output_origin.unsqueeze(0)
    if output_origin.ndim != 3 or output_origin.shape[-1] != 3:
        raise ValueError(
            f"output_origin must have shape (1,T,3) or (T,3), got shape={tuple(output_origin.shape)}"
        )

    occ_pred = sem_grid.clone()
    occ_pred[sem_grid < free_class_id] = 1
    occ_pred[sem_grid == free_class_id] = 0
    occ_pred = occ_pred.permute(2, 1, 0)
    occ_pred = occ_pred[None, None, :].contiguous().float()

    offset = torch.tensor(_pc_range[:3], dtype=torch.float32)[None, None, :]
    scaler = torch.tensor([_voxel_size] * 3, dtype=torch.float32)[None, None, :]
    lidar_tindex = torch.zeros([1, lidar_rays.shape[0]], dtype=torch.float32)

    visible = torch.zeros_like(sem_grid, dtype=torch.bool)
    t_count = int(output_origin.shape[1])
    for t in range(t_count):
        lidar_origin = output_origin[:, t : t + 1, :]
        lidar_endpts = lidar_rays[None] + lidar_origin

        output_origin_render = ((lidar_origin - offset) / scaler).float()
        output_points_render = ((lidar_endpts - offset) / scaler).float()

        with torch.no_grad():
            _, _, coord_index = dvr.render_forward(
                occ_pred.cuda(),
                output_origin_render.cuda(),
                output_points_render.cuda(),
                lidar_tindex.cuda(),
                [1, 16, 200, 200],
                "test",
            )

        coords = coord_index[0, :, :].long().cpu()
        if coords.numel() == 0:
            continue
        valid = (
            (coords[:, 0] >= 0)
            & (coords[:, 0] < sem_grid.shape[0])
            & (coords[:, 1] >= 0)
            & (coords[:, 1] < sem_grid.shape[1])
            & (coords[:, 2] >= 0)
            & (coords[:, 2] < sem_grid.shape[2])
        )
        coords = coords[valid]
        if coords.numel() == 0:
            continue
        if not include_free:
            labels = sem_grid[coords[:, 0], coords[:, 1], coords[:, 2]]
            coords = coords[labels != int(free_class_id)]
        if coords.numel() == 0:
            continue
        visible[coords[:, 0], coords[:, 1], coords[:, 2]] = True

    return visible.numpy().astype(np.uint8)


def calc_rayiou(pcd_pred_list, pcd_gt_list, occ_class_names):
    thresholds = [1, 2, 4]

    gt_cnt = np.zeros([len(occ_class_names)])
    pred_cnt = np.zeros([len(occ_class_names)])
    tp_cnt = np.zeros([len(thresholds), len(occ_class_names)])

    for pcd_pred, pcd_gt in zip(pcd_pred_list, pcd_gt_list):
        for j, threshold in enumerate(thresholds):
            depth_pred = pcd_pred[:, 1]
            depth_gt = pcd_gt[:, 1]
            l1_error = np.abs(depth_pred - depth_gt)
            tp_dist_mask = l1_error < threshold

            for i, cls in enumerate(occ_class_names):
                cls_id = occ_class_names.index(cls)
                cls_mask_pred = pcd_pred[:, 0] == cls_id
                cls_mask_gt = pcd_gt[:, 0] == cls_id

                gt_cnt_i = cls_mask_gt.sum()
                pred_cnt_i = cls_mask_pred.sum()
                if j == 0:
                    gt_cnt[i] += gt_cnt_i
                    pred_cnt[i] += pred_cnt_i

                tp_cls = cls_mask_gt & cls_mask_pred
                tp_mask = np.logical_and(tp_cls, tp_dist_mask)
                tp_cnt[j][i] += tp_mask.sum()

    iou_list = []
    for j, threshold in enumerate(thresholds):
        iou_list.append((tp_cnt[j] / (gt_cnt + pred_cnt - tp_cnt[j]))[:-1])

    return iou_list


class Metric_RayIoU:
    def __init__(self, occ_class_names, thresholds=None):
        if thresholds is None:
            thresholds = [1, 2, 4]
        self.class_names = occ_class_names
        self.thresholds = thresholds
        self.gt_cnt = np.zeros([len(self.class_names)], dtype=np.float64)
        self.pred_cnt = np.zeros([len(self.class_names)], dtype=np.float64)
        self.tp_cnt = np.zeros([len(self.thresholds), len(self.class_names)], dtype=np.float64)
        self.cnt = 0

    def add_batch(self, pcd_pred, pcd_gt):
        self.cnt += 1
        for j, threshold in enumerate(self.thresholds):
            depth_pred = pcd_pred[:, 1]
            depth_gt = pcd_gt[:, 1]
            l1_error = np.abs(depth_pred - depth_gt)
            tp_dist_mask = l1_error < threshold

            for i, _ in enumerate(self.class_names):
                cls_id = i
                cls_mask_pred = pcd_pred[:, 0] == cls_id
                cls_mask_gt = pcd_gt[:, 0] == cls_id

                gt_cnt_i = cls_mask_gt.sum()
                pred_cnt_i = cls_mask_pred.sum()
                if j == 0:
                    self.gt_cnt[i] += gt_cnt_i
                    self.pred_cnt[i] += pred_cnt_i

                tp_cls = cls_mask_gt & cls_mask_pred
                tp_mask = np.logical_and(tp_cls, tp_dist_mask)
                self.tp_cnt[j][i] += tp_mask.sum()

    def count_rayiou(self, print_table=True):
        iou_list = []
        for j, _ in enumerate(self.thresholds):
            iou = (self.tp_cnt[j] / (self.gt_cnt + self.pred_cnt - self.tp_cnt[j]))[:-1]
            iou_list.append(iou)

        rayiou = np.nanmean(iou_list)
        rayiou_0 = np.nanmean(iou_list[0])
        rayiou_1 = np.nanmean(iou_list[1])
        rayiou_2 = np.nanmean(iou_list[2])

        if print_table:
            table = PrettyTable(["Class Names", "RayIoU@1", "RayIoU@2", "RayIoU@4"])
            table.float_format = ".3"
            for i in range(len(self.class_names) - 1):
                table.add_row(
                    [self.class_names[i], iou_list[0][i], iou_list[1][i], iou_list[2][i]],
                    divider=(i == len(self.class_names) - 2),
                )
            table.add_row(["MEAN", rayiou_0, rayiou_1, rayiou_2])
            logging.info("%s", table)

        return {
            "RayIoU": rayiou,
            "RayIoU@1": rayiou_0,
            "RayIoU@2": rayiou_1,
            "RayIoU@4": rayiou_2,
            "iou_list": iou_list,
            "num_samples": self.cnt,
        }


def main_rayiou(
    sem_pred_list,
    sem_gt_list,
    lidar_origin_list,
    occ_class_names,
):
    torch.cuda.empty_cache()

    lidar_rays = generate_lidar_rays()
    lidar_rays = torch.from_numpy(lidar_rays)

    pcd_pred_list, pcd_gt_list = [], []
    for sem_pred, sem_gt, lidar_origins in tqdm(
        zip(sem_pred_list, sem_gt_list, lidar_origin_list),
        ncols=50,
    ):
        sem_pred = torch.from_numpy(np.reshape(sem_pred, [200, 200, 16]))
        sem_gt = torch.from_numpy(np.reshape(sem_gt, [200, 200, 16]))

        pcd_pred = process_one_sample(
            sem_pred,
            lidar_rays,
            lidar_origins,
            occ_class_names=occ_class_names,
        )
        pcd_gt = process_one_sample(
            sem_gt,
            lidar_rays,
            lidar_origins,
            occ_class_names=occ_class_names,
        )

        valid_mask = pcd_gt[:, 0].astype(np.int32) != len(occ_class_names) - 1
        pcd_pred = pcd_pred[valid_mask]
        pcd_gt = pcd_gt[valid_mask]

        assert pcd_pred.shape == pcd_gt.shape
        pcd_pred_list.append(pcd_pred)
        pcd_gt_list.append(pcd_gt)

    iou_list = calc_rayiou(pcd_pred_list, pcd_gt_list, occ_class_names)
    rayiou = np.nanmean(iou_list)
    rayiou_0 = np.nanmean(iou_list[0])
    rayiou_1 = np.nanmean(iou_list[1])
    rayiou_2 = np.nanmean(iou_list[2])

    table = PrettyTable(["Class Names", "RayIoU@1", "RayIoU@2", "RayIoU@4"])
    table.float_format = ".3"

    for i in range(len(occ_class_names) - 1):
        table.add_row(
            [occ_class_names[i], iou_list[0][i], iou_list[1][i], iou_list[2][i]],
            divider=(i == len(occ_class_names) - 2),
        )
    table.add_row(["MEAN", rayiou_0, rayiou_1, rayiou_2])

    logging.info("%s", table)
    torch.cuda.empty_cache()

    return {
        "RayIoU": rayiou,
        "RayIoU@1": rayiou_0,
        "RayIoU@2": rayiou_1,
        "RayIoU@4": rayiou_2,
    }


def main_raypq(
    sem_pred_list,
    sem_gt_list,
    inst_pred_list,
    inst_gt_list,
    lidar_origin_list,
    occ_class_names,
    use_dynamic_id_offset=True,
    include_per_class=False,
    thing_only=False,
):
    torch.cuda.empty_cache()

    eval_metrics_pq = Metric_RayPQ(
        occ_class_names=occ_class_names,
        num_classes=len(occ_class_names),
        thresholds=[1, 2, 4],
        use_dynamic_id_offset=use_dynamic_id_offset,
    )

    lidar_rays = generate_lidar_rays()
    lidar_rays = torch.from_numpy(lidar_rays)

    for sem_pred, sem_gt, inst_pred, inst_gt, lidar_origins in tqdm(
        zip(sem_pred_list, sem_gt_list, inst_pred_list, inst_gt_list, lidar_origin_list),
        ncols=50,
    ):
        sem_pred = torch.from_numpy(np.reshape(sem_pred, [200, 200, 16]))
        sem_gt = torch.from_numpy(np.reshape(sem_gt, [200, 200, 16]))

        inst_pred = torch.from_numpy(np.reshape(inst_pred, [200, 200, 16]))
        inst_gt = torch.from_numpy(np.reshape(inst_gt, [200, 200, 16]))

        pcd_pred = process_one_sample(
            sem_pred,
            lidar_rays,
            lidar_origins,
            instance_pred=inst_pred,
            occ_class_names=occ_class_names,
        )
        pcd_gt = process_one_sample(
            sem_gt,
            lidar_rays,
            lidar_origins,
            instance_pred=inst_gt,
            occ_class_names=occ_class_names,
        )

        valid_mask = pcd_gt[:, 0].astype(np.int32) != len(occ_class_names) - 1
        pcd_pred = pcd_pred[valid_mask]
        pcd_gt = pcd_gt[valid_mask]

        assert pcd_pred.shape == pcd_gt.shape

        sem_gt = pcd_gt[:, 0].astype(np.int32)
        sem_pred = pcd_pred[:, 0].astype(np.int32)

        instances_gt = pcd_gt[:, 1].astype(np.int32)
        instances_pred = pcd_pred[:, 1].astype(np.int32)

        depth_gt = pcd_gt[:, 2]
        depth_pred = pcd_pred[:, 2]
        l1_error = np.abs(depth_pred - depth_gt)

        eval_metrics_pq.add_batch(sem_pred, sem_gt, instances_pred, instances_gt, l1_error)

    torch.cuda.empty_cache()
    return eval_metrics_pq.count_pq(
        include_per_class=include_per_class,
        thing_only=thing_only,
    )


# Backward-compatible aliases for older call sites.
makeTorchExtensionsWritable = make_torch_extensions_writable
resolveDvrSourcePaths = resolve_dvr_source_paths
getDvrModule = get_dvr_module
